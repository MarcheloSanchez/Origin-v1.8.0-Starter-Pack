...
try {
  ...
  if (cache?.cache_date) {
    ...
    metrics = {
      ...
      atomics: dv.pages('"02-Dots"').length ?? 0,
...
try {
  ...
  // Folder distribution
  const onCount = dv.pages('"03-Efforts/On"').length ?? 0;
  const ongoingCount = dv.pages('"03-Efforts/Ongoing"').length ?? 0;
  const simmeringCount = dv.pages('"03-Efforts/Simmering"').length ?? 0;
...
|---------------|-------|---------|
| 🔥 On | ${onCount} | Immediate focus |
| ♻️ Ongoing | ${ongoingCount} | Maintenance work |
| 〰️ Simmering | ${simmeringCount} | Backburner |
`);
...
try {
  ...
  // Check seeds ratio
  const atomics = dv.pages('"02-Dots"').where(p => p.maturity);
