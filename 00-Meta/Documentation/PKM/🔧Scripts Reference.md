...
| **Outputs** | Writes Dataview inline fields (`field:: value`) to `00-Meta/_Metrics Cache.md` |
...
**Performance Impact**: 60-80% dashboard load improvement — dashboards read `dv.page("00-Meta/_Metrics Cache").field_name` with live Dataview fallback if cache is stale.
...
**Analysis**: Content heuristics determine note type (effort, atomic, source, etc.), suggest appropriate folder (`02-Dots/100-Atomics/`, `03-Efforts/`, etc.), recommend tags based on content keywords, and identify related existing notes.
...
| **Outputs** | Updates metadata, moves to `02-Dots/100-Atomics/[subfolder]` |
| **Dependencies** | QuickAdd API, MetadataCache |
...
| **Trigger** | QuickAdd macro |
| **Inputs** | None (operates on active file) |
| **Outputs** | Updates metadata, moves to `03-Efforts/[On|Ongoing|Simmering]` |
| **Dependencies** | QuickAdd API, MetadataCache |

**Processing Time**: ~15-20 seconds. Prompts for deadline and priority, determines folder (On/Ongoing/Simmering) based on deadline proximity, sets status based on urgency.
