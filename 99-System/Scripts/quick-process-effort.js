...
module.exports = async (args) => {
  ...
  try {
    ...
    if (!metadata.tags || metadata.tags.length === 0) {
      ...
      if (!metadata.tags.includes('🚀')) metadata.tags.push('🚀');
    ...
    if (QuickAdd) {
      const folderOptions = [
        `🏃 On (${statusDecision.folder === 'On' ? 'Suggested' : 'Active work'})`,
        `♻️ Ongoing (${statusDecision.folder === 'Ongoing' ? 'Suggested' : 'Recurring'})`,
        `🌊 Simmering (${statusDecision.folder === 'Simmering' ? 'Suggested' : 'Future/low priority'})`,
