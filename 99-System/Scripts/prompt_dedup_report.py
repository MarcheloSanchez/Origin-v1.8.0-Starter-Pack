...
OUTPUT = VAULT / "07-Prompts" / "01-Docs" / "_dedup-report.md"
FUZZY_THRESHOLD = 80
...
def generate_report(groups: list[tuple[list[dict], str, str]]) -> str:
    ...
    if unique_drafts:
        lines.append(f"Found **{len(unique_drafts)}** Draft Copilot prompts "
    ...
    lines.append("| **MERGE_A_TO_B** | B is stub, A has content — "
    ...
    lines.append("")
    lines.append("| Group | Format A (Copilot Draft) | Format B (Prompts_org) "
    ...
    lines.append("")
    lines.append("---")
    lines.append("*Generated by `prompt_dedup_report.py` — review and override "
...
def main():
    ...
    priority = {"MANUAL": 0, "MERGE_A_TO_B": 1, "KEEP_B": 2, "UPGRADE_A": 3,
                "UPGRADE_C": 4, "ARCHIVE": 5, "SKIP": 6}
